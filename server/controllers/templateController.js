...
const createInput = async (content, plain_id) => {
  try {
    ...
    const inputPromises = matches.map(async (match) => {
      ...
      const input = value !== 'date' ? new Input({
        plain_id,
        value,
        type: 0
      }): new Input({
...
const createSelect = async (content, plain_id) => {
  try {
    ...
    const selectPromises = matches.map(async (match) => {
      ...
      const existingSelect = existingSelects.find(select => 
        select.value.join('|') === valueStr
      );
...
exports.createPlain = async (req, res) => {
  try {
    ...
    if (type === 0 || !type) {
      // Plain text type
      if (!content) {
        return res.status(400).json({ msg: "Content is required for plain text type" });
      }
      plain.content = content;
    } else if (type === 1) {
      // Table type
      if (!cells || !Array.isArray(cells)) {
        return res.status(400).json({ msg: "Cells array is required for table type" });
      }
      plain.cells = cells;
    }
